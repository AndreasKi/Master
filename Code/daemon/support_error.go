...
func ErrorCheck(err error, optional_args ...bool) bool {
	...
	if err != nil {
		...
		fmt.Print("\n")
		if fatal {
			log.Fatal(ip_self+":"+port+"> Function "+name+" on line "+strconv.Itoa(line)+" - Failed fatally: ", err)
		} else if show_error {
			log.Println(ip_self+":"+port+">Function "+name+" on line "+strconv.Itoa(line)+" - Had error: ", err)
		}
	}
	return return_val
}
